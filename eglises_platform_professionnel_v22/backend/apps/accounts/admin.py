...
class UtilisateurAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        # Le compte est d'abord enregistré. La confirmation SMS peut être
        # temporairement indisponible sans supprimer le compte.
        with transaction.atomic():
            super().save_model(request, obj, form, change)
            if not change:
                from datetime import timedelta
                VerificationTelephone.objects.filter(
                    utilisateur=obj, utilise=False
                ).update(utilise=True)
                VerificationTelephone.objects.create(
                    utilisateur=obj, code="000000", expire_le=timezone.now() + timedelta(minutes=60)
                )
                ...
                if not all(item.get("ok") for item in resultats):
                    self.message_user(
                        request,
                        "Compte créé en attente de confirmation. Le SMS n'a pas pu être envoyé ; utilisez « Renvoyer le code téléphone » après correction de Supabase.",
                        level=messages.WARNING if 'messages' in globals() else 30,
                    )
