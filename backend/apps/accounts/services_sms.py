...
def normaliser_telephone(telephone: str) -> str:
    ...
    if raw.startswith("+"):
        ...
        if digits.startswith("224"):
            ...
            raise ValueError(
                "Le numéro doit être au format international, par exemple +224622000000."
            )
    if not re.fullmatch(r"\+[1-9]\d{7,14}", digits):
        raise ValueError("Numéro de téléphone invalide. Utilisez le format international +224... .")
    return digits
